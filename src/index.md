...
核心职责：担任核心前端开发，负责首页、商品展示、求购发布、营销活动、个人中心、实人认证、运营工作台等业务页面与组件开发。

核心贡献：

- 瀑布流性能优化：重构首页商品瀑布流，采用虚拟列表 + 图片按需加载 + 差异化渲染策略。FPS 从 45 提升至满帧 58，滚动卡顿率降低 70%，首屏渲染耗时从 1200ms 锐减至 400ms。
- 动态表单架构：负责“求购发布”核心链路，基于 FormBuilder 实现 10+ 字段的条件联动渲染与复杂正则校验，日均稳定支撑 5000+ 次求购发布。
...
- 物料库设计与研发：设计维护基础物料库&业务组件库，封装 30+ 复杂业务组件（动态表单、数据大屏、高阶布局等），支持拖拽生成、属性配置、实时预览全流程
- AI 效能工具落地：基于 MCP 协议，结合 Figma 与 Playwright 搭建前端“设计稿转代码” AI 助手。参与建设自研组件库 llm.txt 文档，实现应用设计还原度达 70%，大幅缩短 UI 还原时间。
- AI Agent 应用开发：参与 AI 应用落地研发，设计并封装支持“文件预览”、“数据预测走势分析”等 AI 对话场景的高阶交互组件。
